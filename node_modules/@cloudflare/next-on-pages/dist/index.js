...
function pLimit(concurrency) {
  ...
  const run = async (fn, resolve9, args) => {
    activeCount++;
  ...
  const enqueue = (fn, resolve9, args) => {
    queue.enqueue(run.bind(void 0, fn, resolve9, args));
    (async () => {
  ...
  const generator = (fn, ...args) => new Promise((resolve9) => {
    enqueue(fn, resolve9, args);
  });
...
async function getPackageVersion(packageName, packageManager) {
  try {
    ...
    if (getPackageVersionRegex) {
      ...
      packageVersion = pm.startsWith("yarn") ? packageInfo?.children?.Version ?? packageInfo?.data?.version : packageInfo?.dependencies?.[packageName]?.version ?? packageInfo?.devDependencies?.[packageName]?.version;
    }
...
async function checkInvalidFunctions(collectedFunctions) {
...
async function tryToFixNotFoundRoute(collectedFunctions) {
  const functionsDir2 = (0, import_path7.resolve)(".vercel", "output", "functions");
  const notFoundDir = (0, import_path7.join)(functionsDir2, "_not-found.func");
  const errorDir = (0, import_path7.join)(functionsDir2, "_error.func");
  ...
  if (invalidNotFound && !invalidError) {
    ...
    const notFoundRscDir = (0, import_path7.join)(functionsDir2, "_not-found.rsc.func");
...
async function prepareAndBuildWorker(outputDir, {
  ...
  const functionsDir2 = (0, import_path8.resolve)(".vercel", "output", "functions");
  if (!await validateDir(functionsDir2)) {
    cliLog(
      "No functions detected (no functions directory generated by Vercel)."
    );
  } else {
    const workerJsDir = (0, import_path8.join)(outputDir, "_worker.js");
    processedFunctions = await processVercelFunctions({
      functionsDir: functionsDir2,
      outputDir,
      workerJsDir,
      nopDistDir: (0, import_path8.join)(workerJsDir, "__next-on-pages-dist__"),
      disableChunksDedup
    });
  }
  ...
  await writeBuildInfo(
    (0, import_path8.join)(outputDir, "_worker.js"),
    staticAssets,
    processedVercelOutput,
    processedFunctions
  );
